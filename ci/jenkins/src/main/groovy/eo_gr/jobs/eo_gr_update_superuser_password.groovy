...
def jobTitle = Defaults.DEFAULT_PREFIX + "-update-superuser-password-job"

freeStyleJob("${jobTitle}") {
    description Defaults.description(
        """This a job contains script for superuser password after EO GR install.""", 
    ...
    parameters {
        choiceParam('ACTIVE_SITE', Defaults.all_env_list, 'Select an Active Site environment to update.')
        choiceParam('PASSIVE_SITE', Defaults.all_env_list_passive_site_default, 'Select a Passive Site environment to update.')
                ...
                --env PYTHONPATH=/workdir \
                 ${TEST_DOCKER_IMAGE} \
                    /bin/bash -c "source venv/bin/activate && \
                     python util_scripts/gr/update_superuser_and_password.py"
        ''')
    }
}

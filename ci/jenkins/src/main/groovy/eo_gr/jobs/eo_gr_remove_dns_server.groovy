...
def jobTitle = Defaults.DEFAULT_PREFIX + "-remove-dns-server-job"

freeStyleJob("${jobTitle}") {
    description Defaults.description("""This job runs script that removes a DNS server (dnsmasq) that makes re-routing for the dedicated EO cluster
    """, 
    ...
    parameters {
        choiceParam('ACTIVE_SITE', Defaults.all_env_list, 'A site (environment) config map of which will be restored to the initial state')
        choiceParam('PASSIVE_SITE', Defaults.all_env_list_passive_site_default, 'A site (environment) config map of which will be restored to the initial state')
                ...
                --env PYTHONPATH=/workdir \
                 ${TEST_DOCKER_IMAGE} \
                    /bin/bash -c "source venv/bin/activate && \
                     python util_scripts/gr/deploy_dnsmasq.py --clean-up"
        ''')
    }
}

...
def jobTitle = Defaults.DEFAULT_PREFIX + "-rv-verify-switchover-before-upgrade-scenario"

pipelineJob(jobTitle) {

    description Defaults.description(
        "This pipeline verifies EO GR switchover before the upgrade on RV (customer-like) environment",
    ...
    parameters {
        ...
        stringParam("GR_STAGE_SHARED_NAME", "swtch-before-upgr", "Provide unique name for CVNFM tests assets")
    ...
    definition {
            cps {
                script(readFileFromWorkspace(Defaults.GR_UPGRADE_PATH + "eo_gr_rv_verify_switchover_before_upgrade.Jenkinsfile"))
                sandbox()
            }
        }

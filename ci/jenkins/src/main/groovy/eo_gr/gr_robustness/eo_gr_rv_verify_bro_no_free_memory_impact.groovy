...
def jobTitle = Defaults.DEFAULT_PREFIX + "-rv-verify-bro-no-free-memory-impact-robustness-scenario"

pipelineJob(jobTitle) {

    description Defaults.description(
        "This pipeline checks for the impact of insufficient memory on the BRO pod in the EO GR",
    ...
    parameters {
        ...
        stringParam("GR_STAGE_SHARED_NAME", "no-free-memory-impact", "Provide unique name for CVNFM tests assets")
    ...
    definition {
            cps {
                script(readFileFromWorkspace(Defaults.GR_ROBUSTNESS_PATH + "eo_gr_rv_verify_bro_no_free_memory_impact.Jenkinsfile"))
                sandbox()
            }
        }

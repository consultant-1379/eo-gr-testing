...
/*
 * This pipeline sets up the environment and verifies idam's leader pod unavailability impact on EO GR switchover scenario:
 * For detailed information on the stages and test scenario please refer to OTC-14221 and EO-177668.
*/
...
pipeline {
    ...
    stages {
        ...
        stage('Running Switchover and delete idam leader db pod after all idam pod recreation on Passive Site Scenario') {
            steps {
                script {
                    build job: 'eo-gr-tests-rv-job',
                    parameters: [
                        ...
                        string(name: 'PASSIVE_SITE', value: env.PASSIVE_SITE),
                        string(name: 'VIM', value: env.VIM),
                        string(name: 'TEST_TAG', value: "switchover_idam_unavailability"),
                        booleanParam(name: 'USE_LOCAL_DNS', value: env.USE_LOCAL_DNS),
                        string(name: 'GR_STAGE_SHARED_NAME', value: env.GR_STAGE_SHARED_NAME),
                        string(name: 'DEPLOYMENT_MANAGER_VERSION', value: env.DEPLOYMENT_MANAGER_VERSION),
                        string(name: 'GERRIT_REFSPEC', value: env.GERRIT_REFSPEC),

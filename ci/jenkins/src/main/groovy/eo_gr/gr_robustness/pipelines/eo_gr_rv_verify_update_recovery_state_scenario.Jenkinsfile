...
/**
 * This pipeline sets up the environment and executes "Verify Update Recovery State Scenario":
 * For detailed information on the stages, please refer to EO-172500.
 */
...
pipeline {
    ...
    stages {
        ...
        stage('Run Verify Update Recovery State Scenario for Passive Site') {
            steps {
                script {
                    build job: 'eo-gr-tests-rv-job',
                    parameters: [
                        ...
                        string(name: 'PASSIVE_SITE', value: env.ACTIVE_SITE),
                        string(name: 'VIM', value: env.VIM),
                        string(name: 'TEST_TAG', value: 'update_recovery_state'),
                        booleanParam(name: 'USE_LOCAL_DNS', value: env.USE_LOCAL_DNS),
                        string(name: 'GR_STAGE_SHARED_NAME', value: env.GR_STAGE_SHARED_NAME),
                        booleanParam(name: 'PRETTY_API_LOGS', value: env.PRETTY_API_LOGS),
                        string(name: 'DEPLOYMENT_MANAGER_VERSION', value: env.DEPLOYMENT_MANAGER_VERSION),
        ...
        stage('Switch DNS settings to the new Active site before switchback') {
            steps {
                script {
                    build job: 'eo-gr-deploy-dns-server-job',
                    parameters: [
        ...
        stage('Make switchover site B -> site A') {
            steps {
                script {
                    build job: 'eo-gr-tests-rv-job',
                    parameters: [

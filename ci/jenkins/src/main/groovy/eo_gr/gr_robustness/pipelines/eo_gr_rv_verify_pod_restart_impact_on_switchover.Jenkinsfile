...
/*
 * This pipeline sets up the environment and executes switchover rollback scenarios (separately):
 * test_switchover_rollback_bur_pod (OTC-14025) or test_switchover_rollback_bur_pod (OTC-14103).
 * For detailed information on the stages, please refer to EO-169164 (OTC-14025) or EO-170891 (OTC-14103).
*/
...
pipeline {
    ...
    stages {
        stage('Set build name') {
            steps {
                script {
                    currentBuild.displayName = "${env.BUILD_NUMBER}_${env.ACTIVE_SITE}_${env.PASSIVE_SITE}_${env.EO_VERSION}_${env.TEST_SCENARIO}"
                }
            }
        }
        ...
        stage('Running rollback test scenario') {
            steps {
                script {
                    build job: 'eo-gr-tests-rv-job',
                    parameters: [
                        ...
                        string(name: 'PASSIVE_SITE', value: env.PASSIVE_SITE),
                        string(name: 'VIM', value: env.VIM),
                        string(name: 'TEST_TAG', value: env.TEST_SCENARIO),
                        booleanParam(name: 'USE_LOCAL_DNS', value: env.USE_LOCAL_DNS),
                        string(name: 'GR_STAGE_SHARED_NAME', value: env.GR_STAGE_SHARED_NAME),
                        booleanParam(name: 'PRETTY_API_LOGS', value: env.PRETTY_API_LOGS),
                        string(name: 'DEPLOYMENT_MANAGER_VERSION', value: env.DEPLOYMENT_MANAGER_VERSION),
        ...
        stage('Running CVNFM and VMVNFM tests phase C') {
            parallel {
                stage('Run CVFNM phase C') {
                    steps {
                        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                            script {
                                build job: 'eo-gr-tests-rv-job',
                                parameters: [
                                    ...
                                    booleanParam(name: 'PRETTY_API_LOGS', value: env.PRETTY_API_LOGS),
                                    string(name: 'DEPLOYMENT_MANAGER_VERSION', value: env.DEPLOYMENT_MANAGER_VERSION),
                                    booleanParam(name: 'RESOURCES_CLEAN_UP', value: true),
                                    string(name: 'OVERRIDE', value: env.OVERRIDE),
                                    booleanParam(name: 'EO_LOG_COLLECTION', value: env.EO_LOG_COLLECTION),
                                    string(name: 'GERRIT_REFSPEC', value: env.GERRIT_REFSPEC),
                                    string(name: 'GERRIT_BRANCH', value: env.GERRIT_BRANCH),

...
def jobTitle = Defaults.DEFAULT_PREFIX + "-rv-verify-disabling-node-networks-robustness-scenario"

pipelineJob(jobTitle) {

    description Defaults.description(
        "This pipeline verifies switchover with disabling networks on bur worker node of Active Site impact scenario.",
    ...
    parameters {
        ...
        stringParam("GR_STAGE_SHARED_NAME", "disable-node-network", "Provide unique name for CVNFM and VMVNFM tests assets or it will generated automatically.")
    ...
    definition {
            cps {
                script(readFileFromWorkspace(Defaults.GR_ROBUSTNESS_PATH + "eo_gr_rv_verify_disabling_node_networks_impact.Jenkinsfile"))
                sandbox()
            }
        }

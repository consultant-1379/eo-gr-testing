...
def jobTitle = Defaults.DEFAULT_PREFIX + "-rv-verify-sftp-unavailability-impact-on-cvnfm-robustness-scenario"

pipelineJob(jobTitle) {

    description Defaults.description(
        "This pipeline verifies SFTP server unavailability impact on CVNFM functionality on the EO GR",
    ...
    definition {
            cps {
                script(readFileFromWorkspace(Defaults.GR_ROBUSTNESS_PATH + "eo_gr_rv_verify_sftp_unavailability_impact_on_cvnfm.Jenkinsfile"))
                sandbox()
            }
        }
    logRotator(50, 50)
